...
async def responder(return_url: str, mensaje: str):
    ...
    if modo == "show":
        ...
        # goto:finish -> cierra el paso del widget para que el bot NO quede colgado y
        # se pueda volver a disparar con el proximo mensaje del cliente. Va detras de
        # una variable (KOMMO_GOTO_FINISH=1 para activarlo); si hiciera un loop, se
        # apaga poniendo la variable en 0. 'step' es obligatorio aunque type=finish.
        if (os.getenv("KOMMO_GOTO_FINISH", "") or "").strip().lower() in ("1", "si", "true", "on"):
            # Kommo corre el bot UNA vez por conversacion. Para responder a TODOS los
            # mensajes de una charla, el bot no debe TERMINAR sino quedar en BUCLE:
            #   goto tipo "question" -> muestra la respuesta y ESPERA el proximo
            #   mensaje del cliente; cuando llega, vuelve al paso 'step' (el del
            #   widget) y re-ejecuta -> responde de nuevo. Seguro: no hace spam,
